/**
 ******************************************************************************
 * @file    SystemAccessibilityService.java
 * @author  Typheye
 * @brief   Accessibility automation service and command dispatcher.
 ******************************************************************************
 * @attention
 *
...
public class SystemAccessibilityService extends AccessibilityService {
    ...
    // Queue of lightweight automation commands submitted from the iApp runtime.
    private static final ArrayList<String> list = new ArrayList<String>();
    private final ArrayList<AccessibilityNodeInfo> list1 = new ArrayList<AccessibilityNodeInfo>();
    private final ArrayList<String> list2 = new ArrayList<String>();
    ...
    private void Event(AccessibilityEvent event) {
        try {
            ...
            // Keep a text snapshot of the active accessibility tree for command lookup.
            sb1.setLength(0);
    ...
    private void getAllText_api25(AccessibilityNodeInfo node, StringBuilder sb) {
        ...
        for (int i = 0; i < j; i++) {
            ...
            // Node path uses child indexes such as "0,2,1" so scripts can target views.
    ...
    private AccessibilityNodeInfo get_api25(AccessibilityNodeInfo node, StringBuilder sb, String id) {
        ...
        for (int i = 0; i < j; i++) {
            ...
            AccessibilityNodeInfo all = node.getChild(i);
            if (all == null) continue;
            log(id.equals(sb2.toString()) + ":" + id + ":" + sb2);
    ...
    protected void onServiceConnected() {
        ...
        new Thread() {
           ...
           */
                    // Drain one command at a time; accessibility APIs must tolerate UI changes between commands.
                    if (list.size() > 0) {
                        try {
                            cmd(list.get(0));
                        } catch (Throwable e) {
    ...
    private void cmd(String cmd) {
        ...
        String all;
        int i2;
        // This parser intentionally stays simple: each line is one command with space/comma arguments.
        for (String all2 : sz) {
            all = all2.trim();
            if (all.startsWith("暂停 ")) {//暂停 1000
                Sleep(Integer.parseInt(all.substring(3)));
            } else if (all.startsWith("//")) {//注释
            } else if (all.startsWith("按键 ")) {//按键 1
                key(Integer.parseInt(all.substring(3)));
            } else if (all.startsWith("提示 ")) {//提示 halo
                Message msg = new Message();
                msg.what = 1;
                msg.obj = all.substring(3);
                handler.sendMessage(msg);
            } else if (all.startsWith("点击 ")) {//点击 100,100
                sz2 = all.substring(3).trim().split(",");
                //log(sz2.length + "点击 " + all.substring(3));
                Click(Integer.parseInt(sz2[0]), Integer.parseInt(sz2[1]));
            } else if (all.startsWith("滑动 ")) {//滑动 600,600,100,100
                sz2 = all.substring(3).trim().split(",");
            ...
            } else if (all.startsWith("输入目标 ")) {//输入目标 1,0,1 halo
            ...
            } else if (all.startsWith("点击目标 ")) {//点击目标 1,0,1
            ...
            } else if (all.startsWith("长按 ")) {//长按 100,100
                sz2 = all.substring(3).trim().split(",");
                //log(sz2.length + "点击 " + all.substring(3));
                LongClick(Integer.parseInt(sz2[0]), Integer.parseInt(sz2[1]));
            } else if (all.startsWith("复制 ")) {//复制 halo
                copy(all.substring(3));
            } else if (all.startsWith("启动 ")) {//启动 com.abc
                openPackageName(all.substring(3));
            } else if (all.startsWith("结束 ")) {//结束 com.abc
                closePackageName(all.substring(3));
            } else if (all.startsWith("正在运行应用")) {//正在运行应用
                runapplist();
            } else if (all.startsWith("应用列表")) {//应用列表
                applist();
            }
